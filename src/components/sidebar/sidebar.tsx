...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <>
      <div
        className={`fixed top-0 left-0 h-full bg-white-800 text-white w-64 p-4 z-40 transform transition-transform duration-300 ease-in-out ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-[#101532]">History</h2>
        ...
        <button
          onClick={onNewChat}
          className="w-full flex items-center justify-center gap-2 px-4 py-2 mb-4 rounded-md bg-[#101532] hover:bg-indigo-600 transition-colors"
        >
...
{threads.length > 0 && (
  <div className="text-xs text-[#101532] mb-2">Loaded {threads.length} threads</div>
)}
    ...
    return (
      <li key={id}>
        <button
          ...
          className={`w-full flex flex-col gap-0 px-3 py-2 rounded-md text-left bg-[#101532] hover:bg-indigo-600 transition ${isActive ? "bg-gray-700" : ""}`}
          title={label}
        >
