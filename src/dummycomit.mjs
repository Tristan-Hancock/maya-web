...
async function markPromptUsed(userId) {
...
async function markDocUsed(userId) {
  const user = await getUser(userId);
  const usage = normalizeUsage(user.usage);

  const next = {
    ...usage,
    docs_used: (usage.docs_used || 0) + 1
  };
...
if (fields.chatContext) {
  ...
  const computedThreadCap = (userRow.subscription_status === "active")
    ...
    const actualThreads = await getThreadCount(anonUser);
...
try {
  if (threadHandle) {
    ...
    // 🚨 Only now do we enforce thread cap
  
    if (computedThreadCap !== null) {
      const actualThreads = await getThreadCount(anonUser);
  
      if (actualThreads >= computedThreadCap) {
        return ok({
...
console.log("Size:", filePart.buffer.length);
  ...
  await markPromptUsed(userId);
  if (hasFile) await markDocUsed(userId);
